...
<body>
  <main class="container-center">
    ...
    <div class="header" style="justify-content: space-between; width:100%">
      ...
      <p class="card__sub">User & Voting Management</p>
    </div>
  </div>
...
<h3>Voting Positions</h3>
<button class="btn btn--primary" onclick="openPositionModal()">+ Add Position</button>
<div class="table-wrap" style="margin-top:12px"><table class="table">
<thead><tr><th>ID</th><th>Position</th><th>Actions</th></tr></thead>
<tbody>
...
<td>
<button class="btn btn--ghost" onclick="openPositionModal(<?= $p['id'] ?>,'<?= htmlspecialchars($p['name'],ENT_QUOTES) ?>')">Edit</button>
<button class="btn btn--danger" onclick="deletePosition(<?= $p['id'] ?>)">Delete</button>
<button class="btn btn--primary" onclick="openCandidateModal(<?= $p['id'] ?>)">Manage Candidates</button>
</td>
</tr>
<?php endforeach; ?>
</tbody>
    ...
    <tbody>
      ...
      while ($pos = $posQ->fetch(PDO::FETCH_ASSOC)) {
          ...
          foreach ($candidates as $c) {
              ...
              echo "<td>
                      <form method='post' onsubmit=\"return confirm('Reset votes for " . htmlspecialchars($position_name, ENT_QUOTES) . "?');\">
                        <input type='hidden' name='reset_position_id' value='{$position_id}'>
