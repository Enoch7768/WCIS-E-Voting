...
foreach ($positions as $pos) {
    $stmt = $pdo->prepare('SELECT * FROM candidates WHERE position_id=? ORDER BY id ASC');
    $stmt->execute([$pos['id']]);
    $candidates_by_position[$pos['id']] = $stmt->fetchAll();
}

// Check user votes per position
$user_votes_stmt = $pdo->prepare('SELECT position_id, candidate_id FROM votes WHERE user_id=?');
  ...
  <div class="card">
    ...
    <div class="candidates">
      <?php foreach($candidates_by_position[$pos['id']] as $c): 
        $candidate_img = $c['image']; // full relative path from DB
